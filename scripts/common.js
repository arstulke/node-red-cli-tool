...
const { readFileAsync, writeFileAsync, httpRequest, toUrlEncoded } = require('./utils');


async function loadConfigFile(projectId) {
    const projectFilePromise = readFileAsync(path.resolve(__dirname, '..', 'project.json'));
    const credentialsFilePromise = readFileAsync(path.resolve(__dirname, '..', 'credentials.json'));

    const projectFile = JSON.parse(await projectFilePromise);
    const projectFlowConfig = projectFile[projectId];
    const flowConfigFilePromise = readFileAsync(path.resolve(__dirname, '..', projectFlowConfig.directory, 'config.json'));
    const flowConfigFile = JSON.parse(await flowConfigFilePromise);

    flowConfigFile.directory = projectFlowConfig.directory;
    flowConfigFile.flowFile = path.resolve(__dirname, '..', projectFlowConfig.directory, flowConfigFile.flowFile);
    ...
    Object.keys(flowConfigFile.stages).forEach((stageKey) => {
        ...
        const serverConfig = Object.assign({}, secretServerConfig, secretProjectConfig, { id: stageKey, serverId: serverId });
